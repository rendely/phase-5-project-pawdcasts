...
class Feed(Resource):
    def get(self):
        ...
        for podcast in followed_podcasts:
            episodes += get_feed_episodes(podcast.feed_url, podcast.title)
...
def get_feed_episodes(url, podcast_title):
    r = requests.get(url)
    ...
    for item in items:
        ...
        episodes.append({'podcast_title': podcast_title, 'title': title, 'date': pubDate, 'mp3': mp3, 'description': description})
